...
bspline_N: BSpline = make_bspline(np.r_[-1, data_t], np.r_[0, data_N], k=2)
bspline_q = bspline_N.derivative()

resample_t_1 = np.arange(0, 2000 + 100, 100, dtype=np.float64)
resample_q_1 = bspline_q(resample_t_1)
resample_N_1 = bspline_N(resample_t_1)

resample_t_2 = np.arange(0, 2000 + 5, 5, dtype=np.float64)
resample_q_2 = bspline_q(resample_t_2)
resample_N_2 = bspline_N(resample_t_2)
...
ax1.plot(resample_t_1, resample_q_1, ls='--', label='BSpline Resample (100 Days)')
ax1.plot(resample_t_2, resample_q_2, ls=':', label='BSpline Resample (5 Days)')
...
ax2.plot(resample_t_1, resample_N_1, ls='--', label='BSpline Resample (100 Days)')
ax2.plot(resample_t_2, resample_N_2, ls=':', label='BSpline Resample (5 Days)')
